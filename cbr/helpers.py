    ...
    if not isinstance(data.index, pd.DatetimeIndex):
        for col in data.columns:
            if any(keyword in str(col) for keyword in ['CDate', 'DateMet', 'D0', 'DT']):
                ...
                data.index = pd.DatetimeIndex(pd.to_datetime(data[col], utc=True)).tz_convert(None)
...
def unstack_groups(data, symbol):
    ...
    if symbol == 'MKR':     
        data = data.groupby([data.index, 'p1'])[['d1', 'd7', 'd30', 'd90', 'd180', 'd360']].first().unstack(level='p1')
    ...
    if isinstance(data.columns, pd.MultiIndex):
        if level_0:
            new_level_0 = data.columns.levels[0].map(
                lambda x: level_0.get(str(x), x)
            )
            data.columns = data.columns.set_levels(new_level_0, level=0)
            
        if level_1:
            new_level_1 = data.columns.levels[1].map(
                lambda x: level_1.get(str(x), x)
            )
